"use client";
import Link from "next/link";
import React, { useState } from "react";

function Searchposts() {
    const [searchkey, setsearchkey] = useState("");
    return (
        <div className="py-2 flex px-3 border dark:text-white dark:bg-black rounded-full mb-2 focus:outline-none border-gray-300 focus:border-gray-600">
            <input
                type="text"
                placeholder="Search"
                value={searchkey}
                onChange={(e) => setsearchkey(e.target.value)}
                onKeyUp={(e) => {
                    if (e.key === "Enter") {
                        window.location.href = `/Post/Search?fild=${searchkey}`;
                    }
                }}
                className="w-full bg-transparent text-sm focus:outline-none dark:text-white dark:bg-black placeholder-gray-500"
            />
            <span className="border my-1 mx-1 sm:block hidden border-gray-400 "></span>

            <Link
                href={{
                    pathname: "/Post/Search",
                    query: { fild: searchkey },
                }}
                className="focus:outline-none focus:border-none pl-1 pr-2"
            >
                <svg
                    className="text-lime-400 h-5 w-auto my-auto fill-current"
                    viewBox="0 0 56.966 56.966"
                    space="preserve"
                    width="512px"
                    height="512px"
                >
                    <path d="M55.146,51.887L41.588,37.786c3.486-4.144,5.396-9.358,5.396-14.786c0-12.682-10.318-23-23-23s-23,10.318-23,23  s10.318,23,23,23c4.761,0,9.298-1.436,13.177-4.162l13.661,14.208c0.571,0.593,1.339,0.92,2.162,0.92  c0.779,0,1.518-0.297,2.079-0.837C56.255,54.982,56.293,53.08,55.146,51.887z M23.984,6c9.374,0,17,7.626,17,17s-7.626,17-17,17  s-17-7.626-17-17S14.61,6,23.984,6z" />
                </svg>
            </Link>
        </div>
    );
}

export default Searchposts;
